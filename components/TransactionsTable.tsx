...
const StatusBadge = ({ pending }: StatusBadgeProps) => {
    const {
        backgroundColor,
        borderColor,
        chipBackgroundColor,
        textColor,
    } = transactionStatusStyles[pending as keyof typeof transactionStatusStyles] || transactionCategoryStyles.default
    return (
        <div className={cn('category-badge', borderColor, chipBackgroundColor)}>
            <div className={cn('size-2 rounded-full', backgroundColor)}/>
            <p className={cn('text-[12px] font-medium', textColor)}>
                {pending === 'false' ? "Success" : "Pending"}
            </p>
        </div>
    )
}
